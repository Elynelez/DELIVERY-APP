...
export const FactureOrderPlatform = ({ rangeItems, data, user, users, conditions, methods, places, setReloadData, URL_SERVER }) => {
    const [form] = Form.useForm();
    const [visible, setVisible] = useState(false);
    ...
    const onFinish = (e) => {
        ...
        var obj = {
            ...
            transaction: {
                ...
                method: e.method,
                order_total: Number(e.total) + Number(e.shipment),
                total_price: e.total,
                shipping_price: e.shipment,
            },
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    ...
                    await axios.post(`${URL_SERVER}/facturation`, obj, {
                        headers: { "Content-Type": "application/json" }
                    });

                    message.success("Contenido facturado exitosamente");
                    setReloadData(true);
                } catch (error) {
                    console.error("Error al subir:", error);
                    message.info("No se pudo completar la operación");
                                ...
                                <div className="input-group-form">
                                    ...
                                    >
                                        <Select
                                            ...
                                            placeholder="Selecciona un departamento"
                                            onChange={(selectedDepartment) => {
                                                const filteredPlace = places.find(obj => obj.state_code == selectedDepartment)
                                                form.setFieldsValue({ department: selectedDepartment, city: filteredPlace.city_code });
                                            }}
                                        >
                                            {places.map((obj, index) => (
                                                <Select.Option key={index} value={obj.state_code}>
                                                    {obj.departamento} - {obj.state_code}
                                                </Select.Option>
                                            ))}
                                        </Select>
                                    </Form.Item>
